...
mod tests {
    use super::*;
    use crate::store::{get_machine_id, Process};
    ...
    fn test_handle_tag_command_add_single_tag() {
        ...
        let process = &store.machine_data.get(&machine_id).unwrap()[0];
    ...
    fn test_handle_tag_command_list_tags_empty() {
        ...
        let result = handle_tag_command(&mut store, store_path, "test_process", None, None, true);
        assert!(result.is_ok());
    }
